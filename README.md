...
一款面向 Ubuntu 的中文桌面点云工作台。可以直接查看、测量、裁剪和分析 PCD、PLY、Odin MAP BIN 与 OLX，也可以连接 Odin1 实时查看并录制点云，以及回放并离线诊断 ROS1、ROS2 和 MCAP bag。点云、设备采集和 Bag 诊断使用同一主窗口内的三个工作台，不再打开独立功能窗口。
...
3. 按 CPU 架构下载 Manifold 官方 MAP BIN 转换器并校验固定 SHA-256；下载失败不影响其他格式。
4. 在桌面和应用程序菜单中创建“点云测量工具”。
...
| 区域 | 用途 |
| --- | --- |
| 左侧工作台导航 | 在“点云测量”“实时采集”“Bag 诊断”之间原位切换，不弹出独立功能窗口 |
...
- 核心算法 UBSan：48 项通过。
- GUI：27 项通过，包括真实 1,514,634 点地图与 OLX 时间轴。
