...
| 设备采集 | ROS2 彩色点云实时预览并录制 OLX；已用真实 Odin1 验证 |
...
5. 选择“SLAM 彩色点云”或“DTOF / 强度”，按需要启用位姿、相机和完成后自动打开。
6. 点击“开始录制”；右侧立即显示可旋转、缩放的实时彩色点云，面板同步显示帧数、累计点数、位姿、图像和刷新率。
...
实时画面最多抽样显示 60,000 点，并通过独立临时文件原子刷新；每一帧的完整点云仍直接写入 OLX。旋转、缩放或预览降采样都不会减少录制文件中的点数。预览临时文件随工作台退出自动清理。
...
- OLX 录制器 Python：13/13 测试通过，覆盖字节序、行填充、intensity、XYZRGBA、坏帧、位姿、图像、实时预览、无 NumPy 回退和会话文件。
- 核心算法 UBSan：48 项通过。
...
先确认状态显示 `ROS2 · CLOUD READY`，并检查所选扫描模式对应的话题正在持续发布。实时画面在收到第一帧后出现；它显示点云而不是相机视频。若计数持续增加但画面没有更新，可查看右下日志中的预览写入错误，完整 OLX 录制不会因预览不可用而停止。
