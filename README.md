...
一款面向 Ubuntu 的中文桌面点云工作台。可以直接查看、测量、裁剪和分析 PCD、PLY、Odin MAP BIN 与 OLX，也可以连接 Odin1 录制 OLX，以及回放并离线诊断 ROS1、ROS2 和 MCAP bag。
...
| 设备采集 | ROS2 话题录制 OLX；已用真实 Odin1 验证 |
...
| Odin1 采集 | 检查 USB/ROS2 状态、启动或停止驱动，从 SLAM/DTOF 话题录制兼容会话 |
...
6. 点击“开始录制”；面板实时显示点云帧、点数、位姿、图像和错误数。
...
- OLX 录制器 Python：12/12 测试通过，覆盖字节序、行填充、intensity、XYZRGBA、坏帧、位姿、图像、无 NumPy 回退和会话文件。
- 核心算法 UBSan：48 项通过。
